...
func setup_user_custom() -> void:
	action_map = {
		...
		"next_song": SignalBus.emit_song_skip_next,
		"previous_song": SignalBus.emit_song_skip_prev,
		...
		"listens_shortcut": SignalBus.emit_show_dynamic_playlists_window,
...
func _unhandled_input(event: InputEvent) -> void:
	for action in action_map:
		if event.is_action_pressed(action, true, true):
